...
test('it should reject if all the params have been tried and nothing calls stopAndResolve', function(assert) {
  let done = assert.async();
  let paramsTried = [];
...
test('it should not try the rest of the items if the first one resolves', function(assert) {
  let done = assert.async();
  let paramsTried = [];
  let params = [1,3,5,6,8,9];
  PromiseTry.findFirst(params, function(nextParam, stopAndResolve /*, tryNext */) {
